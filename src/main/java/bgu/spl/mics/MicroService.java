...
public abstract class MicroService implements Runnable {
    private ConcurrentHashMap<Class<? extends Message>, Callback> messageCallbackConcurrentHashMap;
    ...
    public MicroService(String name) {
    	messageCallbackConcurrentHashMap = new ConcurrentHashMap<>();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        messageCallbackConcurrentHashMap.putIfAbsent(type, callback);
    ...
    public final void run() {
        messageBus.register(this); //Allocates a message-queue for this
        initialize();
        while (!terminate){
            try {
                Message fetchedMessage = messageBus.awaitMessage(this);
                Callback callback = messageCallbackConcurrentHashMap.get(fetchedMessage.getClass());
                if(callback != null) {
                    callback.call(fetchedMessage);
                }
            } catch (InterruptedException e){
                e.printStackTrace();
            }
        }
        messageBus.unregister(this); //Removes the message queue allocated to this micro service, and all the references related to him
    }
}
